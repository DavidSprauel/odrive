...
@section('content')
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header">
                Paniers
                <small>Modifier "{{ $basket->name }}"</small>
    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-6 col-sm-offset-3">
            {{ Form::open(['route' => ['baskets.update', $basket->id], 'method' => 'put', 'files' => true]) }}

            <div class="form-group {{ $errors->has('name') ? 'has-error has-feedback' : '' }}">
                {{ Form::label('name', 'Nom', ['class' => 'control-label']) }}
                {{ Form::text('name', $basket->name, ['class' => 'form-control']) }}
                <small class="text-danger pull-right">{{ $errors->first('name') }}</small>
            </div>

            <div class="form-group {{ $errors->has('description') ? 'has-error has-feedback' : '' }}">
                {{ Form::label('description', 'Description', ['class' => 'control-label']) }}
                {{ Form::textarea('description', $basket->description, ['class' => 'form-control']) }}
            ...
            <div class="row">
                <div class="form-group {{ $errors->has('price') ? 'has-error has-feedback' : '' }} col-sm-6 col-xs-12">
                    ...
                    {{ Form::text('price', $basket->price, ['class' => 'form-control', 'id' => 'price']) }}
                ...
                <div class="form-group {{ $errors->has('status') ? 'has-error has-feedback' : '' }} col-sm-6 col-xs-12">
                    ...
                    {{ Form::select('status', $status, $basket->status, ['class' => 'form-control', 'id' => 'status']) }}
            ...
            <div class="form-group {{ $errors->has('picture') ? 'has-error has-feedback' : '' }}">
                ...
                <img src="{{ $basket->picture }}" alt="{{ $basket->name }}" width="490" />
            </div>
                ...
                @foreach($basket->products as $p)
                    <div class="row productAdd">
                        <div class="form-group {{ $errors->has('product') ? 'has-error has-feedback' : '' }} col-xs-12 col-sm-8">
                            {{ Form::select('products[]', $products, $p->id, ['class' => 'form-control']) }}
                        </div>
                        <div class="form-group {{ $errors->has('product') ? 'has-error has-feedback' : '' }} col-xs-12 col-sm-3">
                            {{ Form::select('quantity[]', $quantity, $p->pivot->quantity, ['class' => 'form-control']) }}
                        </div>
                        <div class="col-sm-1 deleteProduct" title="Supprimer" style="cursor: pointer;margin-top: 3px"><i class="fa fa-fw fa-2x fa-minus-circle text-danger"></i></div>
                    </div>
                @endforeach
            </div>
